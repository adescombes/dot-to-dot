...
class OutputImage():
    ...
    def __init__(self, points, originalWidth, originalHeight, drawLines = False, ensureSpace = True, pdfOutput = None, jpgOutput = None):
        ...
        if not pdfOutput:
            pdfOutput = "./static/processed/dots.pdf"
        if not jpgOutput:
            jpgOutput = "./static/processed/dots.jpg"
    ...
    def setInitialValuesFromBase(self, base, fontSize):
        ...
        self.font = ImageFont.truetype("fonts/open-sans.ttf", int(self.fontSize))
    ...
    def saveImage(self, path = None):
        if not path:
            path = './static/processed/dots.jpg'
    ...
    def drawAsPdf(self, drawLines, path):
        ps = cairo.PDFSurface('./static/processed/dots.pdf', int(self.fullWidth), int(self.fullHeight))
        cr = cairo.Context(ps)
